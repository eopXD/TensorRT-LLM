...
class TestStatsSerializer:
    ...
    def test_serializer_emits_v2_suspend_resume_counters(self):
    ...
    def test_serializer_v2_suspend_resume_counters_default_zero(self):
        """A report with no preemption still emits the keys as 0 (never omitted)."""
        iter_stats = _make_mock_iteration_stats()
        by_window = _make_mock_kv_iter_stats(window_size=16)
        pool_group_stats = _make_mock_kv_iter_stats(window_size=16)[16]
        kv_iter = KVCacheV2IterationStatsReport(
        ...
        d = json.loads(BaseWorker._stats_serializer((iter_stats, None, kv_iter)))

        assert d["iterSuspendedRequests"] == 0
        assert d["iterResumedRequests"] == 0
