...
Two things make a naive version of this test worthless, and both are asserted
against here:

* an adapter whose keys the loader does not recognize is warned about and
  skipped, so it loads clean and does nothing. A run where "LoRA changed the
  output" is never checked cannot tell that apart from success.
* the vision tower takes no LoRA in any VLM in this repo. Without a control
  that stays inert, "the output changed" is not evidence that the intended
  modules are what changed it.
...
@pytest.mark.parametrize("lora_mode", ["eager", "cudagraph"])
def test_nemotron35_vl_multi_lora_with_images(tp_size, lora_mode, model_dir):
    """Three adapters and a no-LoRA row in one batch, every row carrying an image.

    tp1 is the functional gate; tp2 is the sharding gate. The two differ in ways
    that reach LoRA directly: with `num_key_value_heads=2` a tp2 rank holds one
...
@pytest.mark.skip_less_device_memory(140000)
def test_nemotron35_vl_vision_only_lora_is_inert(model_dir):
    """Negative control: a vision-tower adapter must change nothing.

    No VLM in this repo routes `lora_params` to a vision encoder. Pinning that
    here is what makes the positive assertions above mean "the language-model
    modules changed the output" rather than "something changed the output".
    """
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        # A real adapter still has to be present so the engine sizes its LoRA
        # modules; the vision one is the request under test.
        real = create_nemotron35_lora_adapter(
            os.path.join(tmpdir, "real"), model_dir, lora_rank=8, seed=0
        )
        vision = create_vision_only_lora_adapter(os.path.join(tmpdir, "vision"), model_dir)

        llm = LLM(
            model=model_dir,
            lora_config=_lora_config([real]),
        ...
        try:
            sampling_params = SamplingParams(max_tokens=24, temperature=0.0, top_k=1)
            inputs = _image_prompts(llm, model_dir, 1)
            base_tokens = _tokens(llm.generate(inputs, sampling_params, lora_request=None))[0]

            vision_tokens = _tokens(
                llm.generate(
                    _image_prompts(llm, model_dir, 1),
                    sampling_params,
                    lora_request=[LoRARequest("vision-only", 9, vision)],
                )
            )[0]

            assert vision_tokens == base_tokens, (
                "a vision-tower-only adapter changed the output. Either LoRA "
                "reached the vision encoder (it should not) or the adapter's "
                "keys were mapped onto language-model modules"
            )
        finally:
            llm.shutdown()
