...
class SequenceInfo:
    ...
    def nest_sequences(
        ...
        **extra_args: Dict[str, Union[torch.Tensor, Sequence[torch.Tensor]]],
        ...
        if cache_loc_per_pool is not None:
            ...
            for pool_idx in range(num_pools_active):
                ...
                if cu_num_pages_per_pool is not None:
                    ...
                    if pool_idx != 0:
                        # Use per-pool last_page_len when supplied (window-capped
                        # for SWA under front-eviction), otherwise fall back to
                        # the global lpl_host.
                        if last_page_len_per_pool is not None:
                            self._stage_arg(
                                f"last_page_len{suffix}", last_page_len_per_pool[pool_idx]
                            )
                        else:
        ...
        # update sequence length with cache.  Pool 0 always carries the
        # unclamped global value (input_pos + seq_len); non-zero pools carry
        ...
        if self.num_window_groups >= 2:
            for pool_idx in range(1, self.num_window_groups):
                suffix = f"_g{pool_idx}"
                if seq_len_with_cache_per_pool is not None:
                    self._stage_arg(
                        f"seq_len_with_cache{suffix}",
                        seq_len_with_cache_per_pool[pool_idx],
                    )
                else:
