...
class KVCacheV2IterationStatsReport:
    ...
    # Per-request suspend/resume (preemption) counts for the iteration window. These are
    # manager-level (not per-pool-group): suspend/resume is a whole-request state-machine
    # event, so they are surfaced as top-level iteration keys rather than inside
...
def append_kv_cache_iteration_stats(stats_dict: dict, kv_iter_stats) -> None:
    ...
    # Manager-level suspend/resume counters (V2 only): top-level iteration keys, not
    # nested per pool group. >0 means the preemption state machine fired this window.
