...
def test_suspend_resume_counters_reset_each_iteration(resource_guard) -> None:
    """Counters are per-iteration deltas: a drain resets them to 0."""
    manager, request = _make_active_request(resource_guard)
    ...
    first = manager.get_iteration_stats()
    assert first is not None
    assert (first.suspended_requests, first.resumed_requests) == (1, 1)

    # No suspend/resume in the next window -> both counters are back to 0.
    second = manager.get_iteration_stats()
    assert second is not None
    assert (second.suspended_requests, second.resumed_requests) == (0, 0)


def test_suspend_resume_counters_absent_when_stats_disabled(resource_guard) -> None:
    """With stats disabled the state machine still runs but reports nothing."""
    request = _StatsRequest(1, list(range(8)), context_remaining_length=8)
    manager = resource_guard(_create_manager(gpu_bytes=8 << 20, enable_stats=False), request)
